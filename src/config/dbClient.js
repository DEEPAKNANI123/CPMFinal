...
const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || (import.meta.env.PROD ? '/api/db' : 'http://localhost:5000/api/db');
...
export const db = {
  ...
  storage: {
    from: (bucket) => ({
      ...
      getPublicUrl: (path) => ({ data: { publicUrl: `${BACKEND_URL.replace('/api/db', '')}/uploads/${bucket}/${path}` } }),
      remove: async (paths) => {
        try {
